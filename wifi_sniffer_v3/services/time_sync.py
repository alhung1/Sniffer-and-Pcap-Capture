...
Time Sync Service
=================
Synchronises OpenWrt system clock with the local PC.
"""
...
class TimeSyncService:
    """Handles PC <-> OpenWrt time comparison and sync."""
    ...
    def sync_time(self) -> Tuple[bool, str]:
        """Set OpenWrt date to current PC time."""
        try:
            ...
            if ok and stdout.strip():
                try:
                    ...
                    logger.info("Time offset: %.1fs", self.status["offset_seconds"])
                except ValueError as e:
                    logger.debug("Could not parse OpenWrt time: %s", e)
    ...
    def get_time_info(self) -> Dict[str, Any]:
        """Return current PC time, OpenWrt time and offset."""
        pc_time = datetime.now()
        ok, stdout, _ = run_ssh_command("date '+%Y-%m-%d %H:%M:%S'", timeout=10)
